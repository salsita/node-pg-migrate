...
| `schemaIsDefault`           | `boolean`                                   | Marks `schema` as a default your code filled in, as the CLI does without `--schema`: it still sets the `search_path`, but a run that finds no history there is refused when another schema holds one. See [Migration History](cli#migration-history)                                                    |
