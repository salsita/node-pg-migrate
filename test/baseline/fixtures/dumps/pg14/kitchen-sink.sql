...
\restrict ejO5HVg1t3RdcTPlq8Gu5HAkOtL9cIVCT4Yiorb0fbdB37ASxsbb1fTA7zewpjN
...
CREATE VIEW "Sink Área"."From; View" AS
 SELECT "Order; Lines"."Id",
    "Order; Lines"."select" AS "from"
...
CREATE VIEW kitchen.active_customers AS
 SELECT customers.id,
  ...
  WHERE (customers.mood <> 'sad'::kitchen.mood);
...
CREATE VIEW kitchen.cheap_products WITH (security_barrier='true') AS
 SELECT products.id,
  ...
  WHERE ((products.price)::numeric < (10)::numeric)
...
CREATE VIEW kitchen.happy_customers AS
 SELECT active_customers.id,
    active_customers.full_name
   FROM kitchen.active_customers
  WHERE (active_customers.mood = ANY (ARRAY['happy'::kitchen.mood, 'ecstatic'::kitchen.mood]));
...
CREATE VIEW kitchen.referral_chain AS
 ...
 SELECT referral_chain.customer_id,
    referral_chain.root_id,
    referral_chain.depth
   FROM referral_chain;
...
\unrestrict ejO5HVg1t3RdcTPlq8Gu5HAkOtL9cIVCT4Yiorb0fbdB37ASxsbb1fTA7zewpjN
