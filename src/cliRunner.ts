 ...
 */
export async function tryImport<TModule = unknown>(
  moduleName: string
...
/**
 * Runs the CLI for database migrations, parsing arguments and environment variables,
 * loading configuration, and executing the requested migration action (`up`, `down`, `create`, `redo`).
 *
 * @param {string[]} argv - The command-line arguments array (typically `process.argv.slice(2)`).
 * @param {NodeJS.ProcessEnv} env - The environment variables object (typically `process.env`).
 * @returns {Promise<number>} A promise that resolves to the process exit code (0 for success, 1 for error).
 */
export async function runCli(
  ...
  const parser = yargs(argv)
    ...
    .options({
      ...
      [migrationFileLanguageArg]: {
        alias: 'j',
        defaultDescription:
          'last one used or "js" if there is no migration yet',
        choices: ['js', 'ts', 'sql'],
        describe:
  ...
  if (parsedArgv.help || parsedArgv._.length === 0) {
    parser.showHelp();
    return 1;
  }

  /* Load env before accessing env */
  const envPath = parsedArgv[envPathArg];
  ...
  let MIGRATIONS_DIR = parsedArgv[migrationsDirArg];
  let USE_GLOB = parsedArgv[useGlobArg];
  let DB_CONNECTION: string | ConnectionParameters | ClientConfig | undefined =
    env[parsedArgv[databaseUrlVarArg]];
  let IGNORE_PATTERN = parsedArgv[ignorePatternArg];
  let SCHEMA: string | string[] | undefined = parsedArgv[schemaArg];
  let CREATE_SCHEMA = parsedArgv[createSchemaArg];
  let MIGRATIONS_SCHEMA = parsedArgv[migrationsSchemaArg];
  let CREATE_MIGRATIONS_SCHEMA = parsedArgv[createMigrationsSchemaArg];
  let MIGRATIONS_TABLE = parsedArgv[migrationsTableArg];
  let MIGRATIONS_FILE_LANGUAGE: 'js' | 'ts' | 'sql' | undefined = parsedArgv[
    migrationFileLanguageArg
  ] as 'js' | 'ts' | 'sql' | undefined;
  let MIGRATIONS_FILENAME_FORMAT: FilenameFormat | undefined = parsedArgv[
    migrationFilenameFormatArg
  ] as FilenameFormat | undefined;
  let TEMPLATE_FILE_NAME = parsedArgv[templateFileNameArg];
  let CHECK_ORDER = parsedArgv[checkOrderArg];
  let VERBOSE = parsedArgv[verboseArg];
  let DECAMELIZE = parsedArgv[decamelizeArg];
  let tsconfigPath = parsedArgv[tsconfigArg];
  let useTsNode = parsedArgv[tsNodeArg];
  let useTsx = parsedArgv[tsxArg];
  ...
  function isClientConfig(
    val: unknown
  ): val is ClientConfig & { name?: string } {
    return (
      typeof val === 'object' &&
      val !== null &&
  ...
  if (config && config.has(parsedArgv[configValueArg])) {
    const db = config.get(parsedArgv[configValueArg]);
    readJson(db);
  }

  process.env.SUPPRESS_NO_CONFIG_WARNING = oldSuppressWarning;

  const configFileName: string | undefined = parsedArgv[configFileArg];
  if (configFileName) {
  ...
  const action = parsedArgv._.shift();
  ...
  if (action === 'create') {
    ...
    let newMigrationName =
      parsedArgv._.length > 0 ? parsedArgv._.join('-') : '';
    ...
    return Migration.create(newMigrationName, MIGRATIONS_DIR, {
    ...
    if (!DB_CONNECTION) {
      ...
      if (
        !env[parsedArgv[databaseUrlVarArg]] &&
        (!env.PGHOST || !cp.user || !cp.database)
      ) {
        console.error(
          `The ${parsedArgv[databaseUrlVarArg]} environment variable is not set or incomplete connection parameters are provided.`
        );
        return 1;
      }
    ...
    const dryRun = parsedArgv[dryRunArg];
    if (dryRun) {
      console.log('dry run');
    }

    const singleTransaction = parsedArgv[singleTransactionArg];
    const fake = parsedArgv[fakeArg];
    const TIMESTAMP = parsedArgv[timestampArg];
    const rejectUnauthorized = parsedArgv[rejectUnauthorizedArg];
    const noLock = !parsedArgv[lockArg];
    const lockValue = parsedArgv[lockValueArg];
    if (noLock) {
      console.log('no lock');
    }

    const upDownArg = parsedArgv._.length > 0 ? parsedArgv._[0] : null;
    ...
    if (upDownArg !== null) {
      ...
      if (parsedUpDownArg == upDownArg) {
        ...
        migrationName = parsedArgv._.join('-').replace(/_ /g, '-');
      }
    }
