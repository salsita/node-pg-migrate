...
var Migration = function(path, actions, options){
  ...
  self.applyUp = function(done){
    ...
    self.up(pgm, function(){
      ...
      sql_steps.push( utils.t("INSERT INTO "+options.migrations_table+" (name, run_on) VALUES ('{name}', NOW());\n", { name: self.name }) );
  ...
  self.applyDown = function(done){
    ...
    self.down(pgm, function(){
      var sql = 'BEGIN; '+"\n";
      sql += pgm.getSql();
      sql += utils.t("DELETE FROM "+options.migrations_table+" WHERE name='{name}';\n", { name: self.name });
      sql += 'COMMIT;';
