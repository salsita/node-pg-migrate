...
var Migration = function(migrationPath, actions, options) {
  ...
  self.applyUp = function(done) {
    ...
    function upComplete() {
      ...
      sql_steps.push(utils.t('INSERT INTO "{schema}"."{table}" (name, run_on) VALUES (\'{name}\', NOW());', {
  ...
  self.applyDown = function(done) {
    ...
    function downComplete() {
      ...
      var sql_steps = pgm.getSqlSteps();
      sql_steps.push(utils.t('DELETE FROM "{schema}"."{table}" WHERE name=\'{name}\';', { schema: options.migrations_schema, table: options.migrations_table, name: self.name }));
