...
export const dropFunction = typeShorthands => (
  ...
  template`DROP FUNCTION${
    ifExists ? " IF EXISTS" : ""
  } "${functionName}"${formatParams(functionParams, typeShorthands)}${
    cascade ? " CASCADE" : ""
  };`;
...
export const createFunction = typeShorthands => {
  const _create = (
    ...
    return template`CREATE${
      replace ? " OR REPLACE" : ""
    } FUNCTION "${functionName}"${formatParams(functionParams, typeShorthands)}
  RETURNS ${returns}
...
export const renameFunction = typeShorthands => {
  const _rename = (oldFunctionName, functionParams = [], newFunctionName) =>
    template`ALTER FUNCTION "${oldFunctionName}"${formatParams(
      functionParams,
      typeShorthands
    )} RENAME TO "${newFunctionName}";`;
