...
export const dropSequence = (sequenceName, { ifExists, cascade } = {}) =>
  template`DROP SEQUENCE${ifExists ? " IF EXISTS" : ""} "${sequenceName}"${
    cascade ? " CASCADE" : ""
  };`;

export const createSequence = typeShorthands => {
  const _create = (sequenceName, options) => {
    const { temporary, ifNotExists } = options;
    const clauses = parseOptions(typeShorthands, options);
    return template`CREATE${temporary ? " TEMPORARY" : ""} SEQUENCE${
...
export const alterSequence = typeShorthands => (sequenceName, options) => {
...
// RENAME
export const renameSequence = (sequenceName, newSequenceName) =>
