...
export function dropSequence(sequenceName, { ifExists, cascade } = {}) {
  const ifExistsStr = ifExists ? " IF EXISTS" : "";
  const cascadeStr = cascade ? " CASCADE" : "";
  return template`DROP SEQUENCE${ifExistsStr} "${sequenceName}"${cascadeStr};`;
}

export function createSequence(typeShorthands) {
...
export function alterSequence(typeShorthands) {
...
export function renameSequence(sequenceName, newSequenceName) {
