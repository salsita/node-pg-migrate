...
export function dropMaterializedView(viewName, { ifExists, cascade } = {}) {
  const ifExistsStr = ifExists ? " IF EXISTS" : "";
  const cascadeStr = cascade ? " CASCADE" : "";
  return template`DROP MATERIALIZED VIEW${ifExistsStr} "${viewName}"${cascadeStr};`;
}

export function createMaterializedView(viewName, options, definition) {
  const {
    ifNotExists,
    columns = [],
    tablespace,
...
export function alterMaterializedView(viewName, options) {
...
export function renameMaterializedView(viewName, newViewName) {
...
export function refreshMaterializedView(viewName, { concurrently, data } = {}) {
