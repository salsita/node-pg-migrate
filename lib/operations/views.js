...
export function dropView(viewName, { ifExists, cascade } = {}) {
  const ifExistsStr = ifExists ? " IF EXISTS" : "";
  const cascadeStr = cascade ? " CASCADE" : "";
  return template`DROP VIEW${ifExistsStr} "${viewName}"${cascadeStr};`;
}

export function createView(viewName, options, definition) {
...
export function alterView(viewName, options) {
...
export function alterViewColumn(viewName, columnName, options) {
...
export function renameView(viewName, newViewName) {
