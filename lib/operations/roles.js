...
const formatRoleOptions = (roleOptions = {}) => {
  ...
  if (roleOptions.password !== undefined) {
    options.push(
      ...
      } PASSWORD ${escapeValue(roleOptions.password)}`
    );
  }
  if (roleOptions.valid !== undefined) {
    options.push(
      `VALID UNTIL ${
        roleOptions.valid ? escapeValue(roleOptions.valid) : "'infinity'"
      }`
    );
  }
...
export const create = (roleName, roleOptions = {}) => {
  ...
  return template`CREATE ROLE "${roleName}"${
...
export const alter = (roleName, roleOptions = {}) => {
  ...
  return template`ALTER ROLE "${roleName}"${
    options ? ` WITH ${options}` : ""
  };`;
};
