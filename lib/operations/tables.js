...
import { escapeValue, template, quote, applyType, applyTypeAdapters, comment, haveBrackets } from '../utils';
...
const parseReferences = (options) => {
  ...
  const clauses = [
    typeof references === 'string' && haveBrackets(references)
...
export const addColumns = (typeShorthands) => {
  const _add = (tableName, columns) => {
    ...
    const alter = template`ALTER TABLE "${tableName}"\n${formatLines(columnLines, '  ADD ')};`;
