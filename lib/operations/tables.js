...
// TABLE
export function dropTable(tableName, { ifExists, cascade } = {}) {
  const ifExistsStr = ifExists ? " IF EXISTS" : "";
  const cascadeStr = cascade ? " CASCADE" : "";
  return template`DROP TABLE${ifExistsStr} "${tableName}"${cascadeStr};`;
}

export function createTable(typeShorthands) {
...
export function alterTable(tableName, options) {
...
// COLUMNS
export function dropColumns(tableName, columns, { ifExists, cascade } = {}) {
...
export function addColumns(typeShorthands) {
...
export function alterColumn(tableName, columnName, options) {
  const {
    default: defaultValue,
    type,
    collation,
  ...
  const columnCommentStr =
    typeof columnComment !== "undefined"
      ? `\n${comment("COLUMN ", `${tableName}"."${columnName}`, columnComment)}`
      : "";
  const alterStr = `ALTER TABLE "${tableName}"\n${columnsStr};`;
  let templateStr = "";
  if (actions.length > 0) {
    templateStr = alterStr;
  }
  templateStr += columnCommentStr;
  return template`${templateStr}`;
}

export function renameTable(tableName, newName) {
...
export function renameColumn(tableName, columnName, newName) {
...
export function renameConstraint(tableName, constraintName, newName) {
...
export function addConstraint(tableName, constraintName, expression) {
