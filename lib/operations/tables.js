...
const parseConstraints = (table, options, genName) => {
  ...
  if (check) {
    constraints.push(
      `${genName ? `CONSTRAINT "${tableName}_chck" ` : ""}CHECK (${check})`
    );
  }
  if (unique) {
    ...
    (isArrayOfArrays ? uniqueArray : [uniqueArray]).forEach(uniqueSet => {
      ...
      constraints.push(
        `${
          genName ? `CONSTRAINT "${tableName}_uniq_${cols.join("_")}" ` : ""
        }UNIQUE (${quote(cols).join(", ")})`
      );
    });
  }
  if (primaryKey) {
    constraints.push(
      `${genName ? `CONSTRAINT "${tableName}_pkey" ` : ""}PRIMARY KEY (${quote(
        _.isArray(primaryKey) ? primaryKey : [primaryKey]
      ).join(", ")})`
    );
  }
  if (foreignKeys) {
    (_.isArray(foreignKeys) ? foreignKeys : [foreignKeys]).forEach(fk => {
      ...
      constraints.push(
        `${
          genName ? `CONSTRAINT "${tableName}_fk_${cols.join("_")}" ` : ""
        }FOREIGN KEY (${quote(cols).join(", ")}) ${parseReferences(fk)}`
      );
    });
  }
  if (exclude) {
    constraints.push(
      `${genName ? `CONSTRAINT "${tableName}_excl" ` : ""}EXCLUDE ${exclude}`
    );
  }
...
export const drop = (tableName, { ifExists, cascade } = {}) =>
  template`DROP TABLE${ifExists ? " IF EXISTS" : ""} "${tableName}"${
    cascade ? " CASCADE" : ""
  };`;
...
export const create = typeShorthands => {
  const _create = (tableName, columns, options = {}) => {
    ...
    if (dupes.length > 0) {
      throw new Error(
        `There is duplicate constraint definition in table and columns options: ${dupes.join(
          ", "
        )}`
      );
    }
    ...
    const createTable = template`CREATE TABLE${temporary ? " TEMPORARY" : ""}${
...
export const alterColumn = (tableName, columnName, options) => {
  ...
  if (type) {
    actions.push(
      `SET DATA TYPE ${applyTypeAdapters(type)}${
...
export const dropConstraint = (
  ...
  template`ALTER TABLE "${tableName}" DROP CONSTRAINT${
    ifExists ? " IF EXISTS" : ""
  } "${constraintName}"${cascade ? " CASCADE" : ""};`;
