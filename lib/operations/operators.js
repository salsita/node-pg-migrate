...
export const dropOperator = (operatorName, options = {}) => {
  ...
  return `DROP OPERATOR${ifExists ? " IF EXISTS" : ""} ${opSchemalize(
    operatorName
  )}(${schemalize(leftType)}, ${schemalize(rightType)})${
    cascade ? " CASCADE" : ""
  };`;
};

export const createOperatorFamily = (operatorFamilyName, indexMethod) =>
  `CREATE OPERATOR FAMILY ${schemalize(
    operatorFamilyName
  )} USING ${indexMethod};`;
...
export const dropOperatorFamily = (
  ...
  `DROP OPERATOR FAMILY ${ifExists ? " IF EXISTS" : ""} ${schemalize(
    operatorFamilyName
  )} USING ${indexMethod}${cascade ? " CASCADE" : ""};`;
...
const operatorMap = typeShorthands => ({
  ...
  if (String(type).toLowerCase() === "function") {
    ...
    return `OPERATOR ${number} ${opSchemalize(name)}${
      params.length > 0 ? formatParams(params, typeShorthands) : ""
    }`;
  } else if (String(type).toLowerCase() === "operator") {
    return `FUNCTION ${number} ${schemalize(name)}${formatParams(
      params,
      typeShorthands
    )}`;
  }
...
export const removeFromOperatorFamily = typeShorthands => (
  ...
  `ALTER OPERATOR FAMILY ${schemalize(
    operatorFamilyName
  )} USING ${indexMethod} DROP
...
export const renameOperatorFamily = (
  ...
  `ALTER OPERATOR FAMILY ${schemalize(
    oldOperatorFamilyName
  )} USING ${indexMethod} RENAME TO ${schemalize(newOperatorFamilyName)};`;
...
export const dropOperatorClass = (
  ...
  `DROP OPERATOR CLASS ${ifExists ? " IF EXISTS" : ""} ${schemalize(
    operatorClassName
  )} USING ${indexMethod}${cascade ? " CASCADE" : ""};`;
...
export const createOperatorClass = typeShorthands => {
  const _create = (
    ...
    return `CREATE OPERATOR CLASS ${schemalize(operatorClassName)}${
      isDefault ? " DEFAULT" : ""
    } FOR TYPE ${schemalize(applyType(type).type)} USING ${schemalize(
      indexMethod
    )} ${family ? ` FAMILY ${family}` : ""} AS
  ${operatorList.map(operatorMap(typeShorthands)).join(",\n  ")};`;
  };
  _create.reverse = (
    operatorClassName,
...
export const renameOperatorClass = (
  ...
  `ALTER OPERATOR CLASS ${schemalize(
    oldOperatorClassName
  )} USING ${indexMethod} RENAME TO ${schemalize(newOperatorClassName)};`;
