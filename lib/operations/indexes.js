...
function generateIndexName(table, columns, options) {
  return options.name
    ? options.name
    : ((tableName, cols, uniq) => template`${tableName}_${cols}${uniq}_index`)(
        typeof table === "object" ? table.name : table,
        _.isArray(columns) ? columns.join("_") : columns,
...
export const createIndex = (tableName, columns, options = {}) => {
  ...
  const operatorClass = options.operatorClass ? options.operatorClass : "";

  return template`CREATE ${unique} INDEX ${concurrently} "${indexName}" ON "${tableName}"${method} (${columnsString} ${operatorClass})${where};`;
};
