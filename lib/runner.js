    ...
    db
      .select(
        `SELECT obj_description(c.oid) as "comment"
                ...
                }' and c.relkind = 'r' and n.nspname = '${schema}'`
      )
      .then(rows => {
        if (rows.length > 1) {
...
const getRunMigrations = (db, options) => {
  ...
  return db
    .select(
      `SELECT table_name FROM information_schema.tables WHERE table_schema = '${schema}' AND table_name = '${
        options.migrationsTable
      }'`
    )
    .then(
      migrationTables =>
        ...
        db.query(
          template`CREATE TABLE "${{
            schema,
            name: options.migrationsTable
          }}" ( id SERIAL, ${nameColumn} varchar(255) NOT NULL, ${runOnColumn} timestamp NOT NULL)`
        )
    )
    ...
    .then(() =>
      db.column(
        template`SELECT ${nameColumn} FROM "${{
          schema,
          name: options.migrationsTable
        }}" ORDER BY ${runOnColumn}`,
        nameColumn
      )
    )
    .catch(err => {
...
const getMigrationsToRun = (options, runNames, migrations) => {
  if (options.direction === "down") {
    ...
    if (deletedMigrations.length) {
      throw new Error(
        `Definitions of migrations ${deletedMigrations.join(
          ", "
        )} have been deleted.`
      );
    }
        ...
        for (let i = 0; i < len; i += 1) {
          if (runNames[i] !== migrations[i].name) {
            throw new Error(
              `Not run migration ${
                migrations[i].name
              } is preceding already run migration ${runNames[i]}`
            );
          }
        }
      }
