...
const lock = (db, options) => {
  ...
  return db.query('BEGIN')
    .then(() => db.query(template`LOCK "${{ schema, name: options.migrationsTable }}" IN ACCESS EXCLUSIVE MODE`))
    ...
    .then(() => db.query(template`COMMENT ON TABLE "${{ schema, name: options.migrationsTable }}" IS '${lockName}'`))
    .then(() => db.query('COMMIT'));
};

const unlock = (db, options) => {
  const schema = getMigrationTableSchema(options);
  return db.query(template`COMMENT ON TABLE "${{ schema, name: options.migrationsTable }}" IS NULL`);
};
