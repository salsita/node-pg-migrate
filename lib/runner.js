...
const getRunMigrations = (db, options) =>
  db.select(`SELECT table_name FROM information_schema.tables WHERE table_schema = '${options.migrations_schema}' AND table_name = '${options.migrations_table}'`)
    .then(migrationTables =>
      (migrationTables && migrationTables.length === 1)
      || db.query(`CREATE TABLE "${options.migrations_schema}"."${options.migrations_table}" ( id SERIAL, ${nameColumn} varchar(255) NOT NULL, ${runOnColumn} timestamp NOT NULL)`)
    )
    .then(() =>
      db.column(`SELECT ${nameColumn} FROM "${options.migrations_schema}"."${options.migrations_table}" ORDER BY ${runOnColumn}`, nameColumn)
    )
    .catch((err) => {
