  ...
  self.run = function(callback){
    ...
    Step(
      ...
      function fetchCompletedMigrations(err){
        db.query("SELECT * FROM pgmigrations", this)
      },
      function determineMigrationsToRun(err, result){
        ...
        if (migrate_to_index > migrations.length + 1 ) return callback(new Error('Cannot migrate past the last migration defined'));

        console.log( migrate_to_index, current_index );
        ...
        console.log('DB Current state: ' + migrations[current_index-1].name);
        if ( migrate_to_index == 0 ) {
          console.log('Migrate to: CLEAN STATE');
        } else {
          console.log('Migrate to: ' + migrations[migrate_to_index-1].name);  
        }
      ...
      function finish(err){
        if (err) return callback(err);
        console.log(options.direction + ' migration complete');
        callback();
      }
    )
  }
